...
# V3.1.1    10 Feb 2014     Add -t param
...
def parse_options():
    try:
        ...
        opts, args = getopt.getopt(sys.argv[1:], "shp:w:t", long_options)
    ...
    return (split_args, pids_to_show, watch, printTotal)
...
def help():
    ...
    '-t, --print-total                  Print the total in bytes\n'
...
def print_total_memory_usage(total):
    total *= 1024
    if have_pss:
        sys.stdout.write("%d\n" % total)
    else:
        sys.stdout.write("no PSS")
...
if __name__ == '__main__':
    verify_environment()
    split_args, pids_to_show, watch, printTotal = parse_options()
    ...
    if watch is not None:
        ...
        sorted_cmds, shareds, count, total = get_memory_usage( pids_to_show, split_args )
        print_total_memory_usage(total)
    else:
